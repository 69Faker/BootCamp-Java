...
public class CardService
{
    ...
    public void moveToNextColumn(final Long cardId, final List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException
    {
        try {
            ...
            // Uso de Optional para garantir que o card existe
            var dto = optional.orElseThrow(
                    () -> new EntityNotFoundException("O card de id %s não foi encontrado".formatted(cardId))
            );

            // Verificação se o card está bloqueado antes de mover
            if (dto.blocked()) {
                var message = "O card %s está bloqueado, é necesário desbloquea-lo para mover".formatted(cardId);
                throw new CardBlockedException(message);
            }

            // Localiza a coluna atual do card dentro do board
            var currentColumn = boardColumnsInfo.stream()
                    .filter(bc -> bc.id().equals(dto.columnId()))
                    .findFirst()
                    .orElseThrow(() -> new IllegalStateException("O card informado pertence a outro board"));

            // Se o card já está na coluna FINAL, não pode ser movido
            if (currentColumn.kind().equals(FINAL)) {
                throw new CardFinishedException("O card já foi finalizado");
            }

            // Busca a próxima coluna com base no "order" sequencial
    ...
    public void cancel(final Long cardId, final Long cancelColumnId, final List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException
    {
        try {
            ...
            // Mesmo cancelamento exige verificar próxima coluna, garante consistência do fluxo
    ...
    public void block(final Long id, final String reason, final List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException
    {
        try {
            ...
            // Regras de negócio: cards em FINAL ou CANCEL não podem ser bloqueados
    ...
    public void unblock(final Long id, final String reason) throws SQLException
    {
        try {
            ...
            // Apenas cards bloqueados podem ser desbloqueados
            if (!dto.blocked()) {
